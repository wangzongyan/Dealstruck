...
lc.sample <- sample_n(lc_i %>% subset(!loan_status %in% c("Current", "Fully_Paid")), 3000, replace = T)
...
ggsave(filename = "plot_significant.png", plot = plot_significant, path = ".",  
       width = 10, height = 6, dpi = 400)
```
## multi-level Logistic regression model accuracy
```{r, warning= F}
## in-sample error
...
```{r}
  ...
  # sample SVM model"
svmmodel <- svm(lc_svm,"Fully_Paid")

  #our SVM model
newdata <- as.matrix(lc_svm%>% select(-loan_status))
kern.list <- kern_list(lc_svm)
...
# plot classification result
plot_mis_kern <- mis_plot(true = lc_ii$loan_status, pred = pred_svm)
print(plot_mis_kern)
ggsave(filename = "plot_mis2.png", plot = plot_mis2, path = ".",  
       width = 10, height = 6, dpi = 400)
